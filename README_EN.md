...
- 🔌 **Local service bridge (WebUI support)**: `fetch`/XHR requests in pages to `http://127.0.0.1:*` and `localhost` are automatically rerouted through a built-in forwarding channel (custom scheme + CORS allow), so local WebUIs like AnkiConnect (127.0.0.1:8765), Stable Diffusion WebUI or Jupyter work inside KanWindow just like in Chrome.
